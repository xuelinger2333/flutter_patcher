...
await FlutterPatcher.init(
  config: PatcherConfig(maxCrashCount: 1), // 默认
);
```
...
```dart
await FlutterPatcher.init(
  config: PatcherConfig(verifyAfter: const Duration(seconds: 5)), // 默认
);
```
...
调用 [`ActivityManager.getHistoricalProcessExitReasons`](https://developer.android.com/reference/android/app/ActivityManager#getHistoricalProcessExitReasons(java.lang.String,int,int)) 精确区分进程退出原因。查询时用启动阶段记录的 pid 过滤，确保只看到上一次启动的退出记录，而不是更早的历史进程。
...
任意一个钩子触发，等同于一次崩溃，立即写入 `crash_count++` 并删除补丁文件、标记回滚。但由于当前进程的 `libapp.so` 已经加载到内存中，无法在本次启动内切回内置版本——实际恢复仍然要等下次冷启动。这里"立即回滚"的含义是：**立刻在磁盘上完成回滚准备**（删除补丁、写入黑名单、清除 `patch_loading` 标记），确保下次冷启动一定能正常启动，而不是把判定推迟到下次启动时再做。
...
- 跨 APK 升级保留——黑名单的 key 不含 versionCode，所以升级 APK 后仍然生效。虽然 versionCode 不匹配的旧补丁本身就不会加载，但保留黑名单是为了防止一种边界情况：服务端在新版本下错误地复用了旧补丁的 version 字符串和相同的文件（md5 相同），此时黑名单可以阻止这份已知有问题的二进制被再次加载
...
| 时机 | 行为 |
|---|---|
| `Application.attachBaseContext` | 写入 `patch_loading=true` + 当前 pid（同步 `commit`，不能用 `apply`）。pid 用于 Android 11+ 查询 `ApplicationExitInfo` 时过滤出本次启动的退出记录 |
| Dart `FlutterPatcher.init()` | 再写一次 `patch_loading=true`。兜底 native 层 `commit` 失败的极端情况（如磁盘满、进程在 `attachBaseContext` 和 Dart 初始化之间被杀） |
| 首帧渲染 | 启动前台存活计时器 |
| 前台累计存活 N 秒 | `patch_loading=false`，`crash_count=0`，卸载错误钩子 |
| Dart 错误钩子触发（未 verified 期间） | `crash_count++`，立即在磁盘上完成回滚准备（删除补丁 + 入黑名单），实际恢复等下次冷启动 |
| 下次冷启动发现 `patch_loading==true` | API 30+ 用记录的 pid 查 `ApplicationExitInfo` 区分退出原因；API < 30 直接计一次失败 |
...
| 错误码 | 说明 |
|---|---|
| `invalidArgs` | 参数缺失或格式错误 |
| `blacklisted` | 该补丁已在黑名单中 |
| `network` | 下载失败（超时、DNS、连接中断等） |
| `md5Mismatch` | 下载后的文件 MD5 与传入值不一致 |
| `signatureInvalid` | Ed25519 签名校验失败 |
| `ioError` | 文件写入 / 重命名失败 |
| `bsdiffDisabled` | 服务端下发了 bsdiff 增量补丁，但客户端未启用 bsdiff 模式（bsdiff 配置见 [Architecture](https://pub.dev/documentation/flutter_patcher/latest/topics/Architecture-topic.html)） |
| `bsdiffApplyFailed` | bsdiff 增量合并失败（基准文件损坏或版本不匹配） |
| `targetMd5Mismatch` | bsdiff 合并后的完整文件 MD5 与预期不一致 |
| `unknown` | 未归类错误，查看 `r.message` 获取详情 |

> 其中 `bsdiffDisabled`、`bsdiffApplyFailed`、`targetMd5Mismatch` 仅在使用 bsdiff 增量补丁模式时出现。bsdiff 模式的启用和配置见 [Architecture](https://pub.dev/documentation/flutter_patcher/latest/topics/Architecture-topic.html)。
